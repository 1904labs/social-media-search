...
function initializeStoredValues() {
  ...
  setTimeout(() => {
    document
      ...
      .addEventListener("click", resetMediaValues);
    document
      .querySelector("#apply-button")
      .addEventListener("click", applyMediaValues);
  }, 500);
}
...
function applyMediaValues() {
  ...
  for (let key in socialMediaKeys) {
    if (socialMediaKeys.hasOwnProperty(key)) {
      ...
      socialMediaKeys[key] = document.querySelector(
        "#" + labelID
      ).MaterialSwitch.inputElement_.checked;
    }
  }
